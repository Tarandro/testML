...
class Word2Vec(Base_Embedding):
    ...
    def preprocessing_fit_transform(self, x, size_params='small', method_embedding='word2vec'):
        ...
        if self.method_embedding.lower() == 'word2vec':
            ...
            if not os.path.exists(dir_word2vec):
                ...
                build_word2vec_model(list(x.iloc[:, col]), output_dir=dir_word2vec,
    ...
    def save_params(self, outdir_model):
        params_all = dict()
        params_all['name_embedding'] = "word2vec"
    ...
    def load_params(self, params_all, outdir):
        ...
        if self.method_embedding.lower() == 'word2vec' or not os.path.exists(self.method_embedding):
            dir_word2vec = os.path.join(outdir, 'Word2Vec')
            if os.path.exists(dir_word2vec):
                self.method_embedding = os.path.join(dir_word2vec, "word2vec.wordvectors")
            else:
                logger.error("A directory 'Word2Vec' with the model must be provided in {}".format(outdir))
    ...
    for i, doc in tqdm(enumerate(documents), disable=not verbose):
        ...
        try:
            ...
            if len(embedding_vector) < 1:
                ...
                # TODO : add option document vector = average weighted by tf-idf
